...
const Info = () => {
    ...
    const amberAlertList = amberList.map((msg, i) => <p key={i}>{msg.sensor_model} at {msg.datetime}</p>);
    const redAlertList = redList.map((msg, i) => <p key={i}>{msg.sensor_model} at {msg.datetime}</p>);
